...
## Phase 27 — 관심 주제 예약 재생·실패 재시도 폭주 수정

- [x] 재시작 시 지난 예약을 즉시 실행하지 않고 현재 시각+설정 간격으로 재설정.
- [x] 예약 실행 전에 다음 시각을 저장해 실패해도 scheduler poll마다 반복하지 않도록 수정.
- [x] 수동 재시도는 즉시 실행되며 다음 자동 예약만 전진하는 동작 유지.
- [x] 재시작 무재생과 두 번 이상의 poll에 걸친 실패 호출 횟수/시각 회귀 테스트 추가.
- [x] 삭제 시 현재/이전뿐 아니라 해당 관심 주제의 전체 실행 기록을 경로 안전하게 제거하고 다른 주제는 보존.
- [x] UI/가이드/문서에서 최근 상위 10개·정확한 URL 기준 검색 결과 변동임을 명시하고 사실 변화 판정과 구분.
