...
기본값은 무료로 자체 호스팅하는 `RETRIEVER=searxng`입니다. 대신 유료 검색 API를 처음부터 사용하려면 `.env`의 `RETRIEVER`를 아래 서비스 중 하나로 바꾸고 해당 서비스의 API 키를 직접 설정할 수 있습니다.
...
이 프로젝트는 기본적으로 관리할 비밀과 과금 대상이 DeepSeek API 키 하나뿐이지만, 유료 리트리버 선택은 그 원칙의 명시적인 선택적 예외입니다. 선택하면 두 번째 유료 API 키를 관리해야 하며, 모든 검색 섹션에서 해당 서비스가 호출될 수 있어 비용이 빠르게 늘 수 있습니다.

선택한 서비스에서 키를 발급받은 뒤 `.env`의 `RETRIEVER`를 바꾸고 해당 서비스의 키를 설정한 다음 앱을 재배포합니다. 서비스별 키 이름은 `TAVILY_API_KEY`, `SERPER_API_KEY`, `SERPAPI_API_KEY`, `SEARCHAPI_API_KEY`, `EXA_API_KEY`입니다.
...
1. **홈**: 저장된 주제 카드 목록. 진행률(N/M 섹션), 생성일이 보이고, 완료된 주제는 Markdown/Excel/섹션별 ZIP 다운로드 버튼이 나타납니다. 목차 생성 중이거나 실패한 주제는 진행률 대신 상태 배지가 표시됩니다. "삭제"는 언제든 가능(목차 생성 또는 리서치 진행 중인 주제 제외). 히어로의 **서버 로그**에서 최근 1,000개 로그를 확인할 수 있고, 화면은 4초마다 새 로그를 가져옵니다. SearXNG 검색이 예외 또는 빈 출처로 실패하면 응답 없는 엔진의 오류를 바탕으로 봇 차단 가능성을 판정한 WARNING 로그도 여기에 남습니다. 이 진단은 `RETRIEVER=searxng`(기본값)일 때만 남습니다. GPT-Researcher가 실제 검색에는 LLM이 만든 여러 하위 쿼리를 사용하므로, 이 진단은 실패한 하위 쿼리를 정확히 재현한 결과가 아니라 같은 SearXNG 인스턴스와 시점에서 원본 섹션 쿼리로 확인한 근사치입니다.
